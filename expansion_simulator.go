...
func (sim *WorldSimulator) SimulateFactionExpansion(faction *FactionState, domains []*DomainState, ticks int) []float64 {
	...
	// Считаем текущее влияние фракции в домене
	sourceInfluence := 0.0012 * sim.factionWealthIndex(faction)
	u := make([]float64, n)
	for i := 0; i < n; i++ {
		...
		if domains[i].ControlledBy == faction.ID && u[i] < 1.0 {
			// Мягкий переток с учётом factionWealthIndex
			u[i] += sourceInfluence * (1.0 - u[i])
		}
		u[i] = clamp(u[i], MinInfluence, 1.0)
	}

	// Считаем коэффициент диффузи (насколько быстро влияние распространяется на соседние домены)
